...
mod taproot_regtest_tests {
    ...
    struct RegtestClient<'c>(&'c Client);
    const WAL_NAME: &'static str = "cs_e2e_test";

    fn setup_wallets<'c>(rpc: &'c Client) -> Result<RegtestClient<'c>, Error> {
        match rpc.load_wallet(WAL_NAME) {
            Ok(_) => Ok(RegtestClient(rpc)),
            Err(_) => {
                rpc.create_wallet(WAL_NAME, Some(true), Some(true), None, Some(false))?;
                Ok(RegtestClient(rpc))
            }
        }
    }
    ...
    fn taproot_key_spend() -> Result<(), Error> {
        ...
        let mut funder_keypair = schnorr::KeyPair::from_secret_key(&s, sk);
        ...
        let _hashes = rpc.generate_to_address(100, &funder_address)?;

        // send transaction spending funds to Musig2 contract
        let txid_out = rpc.send_raw_transaction(&tx)?;

        let _hashes = rpc.generate_to_address(1, &funder_address)?;
        ...
        let _contract_address = contract.address(&s).unwrap();
    ...
    #[test]
    fn taproot_script_spend() -> Result<(), Error> {
        let url = "http://127.0.0.1:28332";
        let user = "coinswap_rpc_test";
        let pass = "coinswap_rpc_pass";
        ...
        let keypair = schnorr::KeyPair::from_secret_key(&s, main_sk);
        let oth_sk = SecretKey::from_slice(&[
            0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
            0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x6e, 0x6f, 0x6f, 0x6d, 0x6d, 0x6f, 0x6f, 0x6e,
        ])?;
        let oth_keypair = schnorr::KeyPair::from_secret_key(&s, oth_sk);
        ...
        let timelock_keypair = schnorr::KeyPair::from_secret_key(&s, timelock_sk);
        let spend_keypair = schnorr::KeyPair::new(&s, &mut rng);
